        ...
        if (ent.isBlob()) {
            ...
            if (!filter || filter(newPrefix)) {
                $this.repo.getTree(ent.oid()).then((nextTree) => {
                    repoTreeWalkImpl($this,newPrefix,nextTree,callback,filter);
                });
            }
        }
    }
...
function repoTreeWalk($this,callback,filter,targetTree) {
    ...
    return repoTreeGetTree($this,targetTree).then((tree) => {
        repoTreeWalkImpl($this,targetTree,tree,callback,filter);
    });
}
...
// Walks the tree and calls callback(path,name,stream) for each blob. If
// specified, filter(path) -> false heads off a particular path branch.
RepoTree.prototype.walk = function(callback,filter) {
    repoTreeGetConfig(this,'target-tree',false)
        .then((targetTree) => { repoTreeWalk(this,callback,filter,targetTree); },
              (err) => { repoTreeWalk(this,callback,filter); });
};
...
// Walks the tree and calls callback(path,name,stream) for each blob. If
// specified, filter(path) -> false heads off a particular branch path.
